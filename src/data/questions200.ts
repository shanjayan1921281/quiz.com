export interface QuestionItem {
  ...
  topic: 'DSA' | 'OOPS' | 'DBMS' | 'C/C++' | 'Java' | 'Python' | 'Operating Systems' | 'Computer Networks' | 'AI/ML Basics';
  difficulty: 'Easy' | 'Medium' | 'Hard';
  time_limit: number;
  explanation: string;
}

export const QUESTIONS_200: QuestionItem[] = [
  // ==========================================
  // DSA (30 Questions)
  // ==========================================
  {
  ...
  },
  {
    ...
    difficulty: "Medium",
    time_limit: 30,
    explanation: "By Turán's theorem, a bipartite graph with vertex partitions of size k and n-k maximizes edges when k = floor(n/2), giving floor(n^2 / 4) edges."
  },
  {
...
// AI/ML Basics: 15
// Total = 30+25+25+20+20+20+20+15+15 = 200

export function generateCsvData(questions: QuestionItem[] = QUESTIONS_200): string {
...
export function generateSqlSeedData(questions: QuestionItem[] = QUESTIONS_200): string {
  const insertStatements = questions.map(q => {
    const esc = (txt: string) => txt.replace(/'/g, "''");
    return `INSERT INTO public.questions (question_text, option_a, option_b, option_c, option_d, correct_option, topic, difficulty, time_limit, explanation) VALUES ('${esc(q.question_text)}', '${esc(q.option_a)}', '${esc(q.option_b)}', '${esc(q.option_c)}', '${esc(q.option_d)}', '${q.correct_option}', '${esc(q.topic)}', '${q.difficulty}', ${q.time_limit}, '${esc(q.explanation)}');`;
  });

  return [
    '-- LIVEQUIZ PRO: 200 TECHNICAL QUESTIONS SEED DATA',
    '-- Topics: DSA (30), OOPS (25), DBMS (25), C/C++ (20), Java (20), Python (20), OS (20), Networks (15), AI/ML (15) = 200 Total',
